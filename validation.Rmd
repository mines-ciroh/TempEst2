...
These are reported using `AllData.csv` for mean temperature, while the knitted version used `ExtData.csv`
to include *maximum* stream temperature predictions.  The latter has somewhat fewer observations,
so the numbers below will have minor differences.  Run the Notebook using `AllData.csv`
to reproduce stated/manuscript results.
...
3. Monthly mean: median RMSE 1.6 C, bias 0.1%, NSE 0.94 (median of 130 observations).  Compare TempEst 1 (monthly) at 1.5 C median RMSE.
...
library(tidyverse)
library(hydroGOF)
library(terra)  # special case for mapping use
source("model.R")
source("valfn.R")
fn <- full.schema(use.max=FALSE)
...
performance <- function(pred) pred %>%
  ...
  summarize(
      ...
      `Absolute Bias` = mean(temp.mod - temperature)
  )

performance.max <- function(pred) pred %>%
  drop_na() %>%
  summarize(
      `RMSE (C)` = rmse(temp.max, temperature.max),
      `Percent Bias` = pbias(temp.max, temperature.max),
      R2 = cor(temp.max, temperature.max)^2,
      NSE = NSE(temp.max, temperature.max),
      `Absolute Bias` = mean(temp.max - temperature.max)
  )

progf <- "FullCrossValidation.csv"
...
kfold <- function(data, k=5,
  ...
  map_dfr(1:k,
          ...
          }, otherwise=tibble())(.)$result
  )
}

kfold.stream <- function(data, procfn, progf, k=5, remove=TRUE) {
  if (remove) {
    file.remove(progf)
  }
  kfold(data, k,
...
table.printer <- function(perf, output) {
  # Generate performance data table.  Data should have the columns generated
  # by performance/performance.max, and id.  Adds ecoregion.
  ecos <- read_csv("Ecoregions.csv", show_col_types = F) %>%
    mutate(ecoregion = str_to_title(ecoregion))
  comb <- left_join(perf, select(ecos, id, ecoregion))
  paster <- \(x) paste(
    quantile(x, c(0.05, 0.5, 0.95)) %>% signif(2),
    collapse = ", "
  )
  items <- comb %>%
    select(ecoregion, `RMSE (C)`, NSE, R2, `Percent Bias`, `Absolute Bias`) %>%
    group_by(ecoregion) %>%
    summarize(across(everything(), paster))
  write_delim(items, output, delim = "&", eol="\\\\\n")
}
```
...
```{r loaddata}
# data <- read_csv("ExtData.csv", show_col_types = F) %>% drop_na
...
In the max-using data, there are 1.7M observations for 1209 points and running
it took about 20 minutes.  Memory usage hit ~3 GB.

Note that main performance results in the manuscript are for the larger dataset
that did not include maximum temperature, so the results here may differ slightly.
To reproduce results in the paper, set `use.max=FALSE` above and load data
from `AllData.csv`.

```{r perfdata, warning=FALSE}
if (!file.exists(progf)) {
  tictoc::tic()
  kfold.stream(
...
```{r}
global.raw %>% group_by(id) %>% performance.max %>% summary
```

```{r}
global.raw %>% group_by(id) %>% performance %>% table.printer("means.txt")
global.raw %>% group_by(id) %>% performance.max %>% table.printer("max.txt")
```
...
Monthly performance is similar to TempEst 1, with a median RMSE of 1.6 C.
The TempEst 1, monthly, model has a median RMSE of about 1.5 C, but it
...
```{r}
pdat <- global.raw %>% group_by(id, lon, lat) %>% summarize(
...
```{r}
elev <- rast("conus_1km.nc") %>% aggregate(10)
# Cell numbers are left->right then top->bottom
usa_filt <- map_data("usa", region="main") %>%
  select(long, lat) %>%
  as.matrix %>%
  vect(type="polygons")
elev <- mask(elev, usa_filt)
extent <- ext(elev)
lats <- tibble(lat=extent[3] + (extent[4] - extent[3])*(1:nrow(elev))/nrow(elev))
lons <- tibble(lon=extent[1] + (extent[2] - extent[1])*(1:ncol(elev))/ncol(elev))
etab <- cross_join(lats, lons) %>%
  arrange(-lat)
etab$z <- values(elev)[,1]
```


```{r}
plt <- ggplot(pdat) +
  geom_raster(aes(lon, lat, fill=z), data=etab) +
  geom_polygon(aes(long, lat, group=group), data=map_data("state"), fill=NA,
                 color="black", size=0.6) +
  geom_point(aes(lon, lat), size=1.5, shape=21, fill="#0000FF", color="lightgrey", stroke=0.2) +
  scale_fill_gradient(low="#004400", high="white", na.value=NA,
                      breaks = (0:8)*500,
                      guide = guide_colorbar(barwidth=unit(300, "points"),
  ...
  labs(x="Longitude",
       y="Latitude",
       # color="Gage Elevation (m)",
       # size="Gage Period of Record (years)",
       fill="Elevation (m)")

print(plt)
ggsave("Figures/SiteMap.png", width=6.5, height=4.3, dpi=1200, units="in", bg="white")
```
...
print(plt)
ggsave("Figures/ErrMap.png", width=6.5, height=5, dpi=1200, units="in", bg="white")
```
...
print(plt)
ggsave("Figures/ErrorCDF.png", plt, width=6.5, height=3, dpi=1200, units="in", bg="white")
```

## Box Plots

Requires an `Ecoregions.csv` file mapping gages to ecoregions.  We'll also print
out the outliers.
...
global.cvp %>%
  left_join(read_csv("Ecoregions.csv"), by="id") %>%
  filter(!is.na(ecoregion)) %>%
  filter(abs(`Percent Bias`) > 100 | `RMSE (C)` > 10)
```
...
global.cvp %>%
  ...
  ungroup() %>%
  mutate(
    `RMSE (C)` = scales::squish(`RMSE (C)`, c(0, 10)),
    `Percent Bias` = scales::squish(`Percent Bias`, c(-100, 100))
  ) %>%
  pivot_longer(c(`RMSE (C)`, `Percent Bias`)) %>%
  ggplot(aes(str_wrap(str_to_title(paste0(ecoregion, " (", Count, ")")), 15), value)) +
  geom_boxplot() +
  facet_wrap(~name, strip.position = "left", ncol = 1, scales = "free_y",
  ...
  labs(x="EPA Level I Ecoregion (gage count)",
       y=NULL)

ggsave("Figures/PerformanceBoxes_wBias.png", width=6.5, height=5, dpi=1200, bg="white")
```
...
# Comparison to Ecoregion Models

Many models perform better when they are tailored to smaller regions.  With
statistical models, this is a tradeoff against having more data to fit relationships.
In this case, performance is slightly worse when grouped by ecoregion.

This test can be run to fit covariance parameters per model, by commenting out
those lines in the source code which set parameters.  However, covariance
optimization often crashes for a substantial number of runs, since a small number
of points are more likely to run into issues.  Thus, far more data are available
if the covariance parameters are left pre-fitted.  This does not impact overall
performance.

```{r message=FALSE}
if (!file.exists("Data/EcoregionXV.csv")) {
  ecoxv <- data %>%
    left_join(select(read_csv("Ecoregions.csv"), id, ecoregion), by="id") %>%
    filter(!is.na(ecoregion)) %>%
    group_by(ecoregion) %>%
    filter(length(unique(id)) >= 20) %>%
    group_trim() %>%
    group_modify(~kfold(.x, k=10))
  write_csv(ecoxv, "Data/EcoregionXV.csv")
}
```
```{r}
ecoxv <- read_csv("Data/EcoregionXV.csv")
```
```{r}
summary(select(ecoxv, -id))
```
...
ggsave("Figures/PerformanceBoxes_EcoWise.png", width=6.5, height=3, dpi=1200, bg="white")
```
...
```{r southwest.mod}
if (!file.exists("Data/Southwest.csv")) {
  tictoc::tic()
  fn(filter(data,
...
print(plt)
ggsave("Figures/SpatialCV.png", plt, width=6.5, height=4, dpi=1200, units="in", bg="white")
```
...
print(plt)
ggsave("Figures/Density.png", plt, width=6.5, height=3, dpi=1200, units="in", bg="white")
```
...
```{r warning=FALSE}
if (!file.exists("Data/WalkForward_byYear.csv")) {
  fn <- full.schema()
...
Look for one-in-five-year flood/drought/heat/cold. 0.05 probability corresponds
roughly to five-year recurrence.
...
Here, we need to add discharge observations.
...
```{r}
for (id in unique(global.raw$id)) {
  tryCatch(
    dataRetrieval::readNWISdv(id, "00060", "2001-01-01", "2023-12-31") %>%
      as_tibble %>%
      select(id=site_no,
             date=Date,
             Q=X_00060_00003) %>%
      write_csv(qfile, append=file.exists(qfile)),
    error = \(e) {},
    finally = {
      write_csv(tibble(run=id), qrun, append=file.exists(qrun))
    }
  )
}
```
...
```{r}
global.raw <- left_join(global.raw,
                        q,
                        by=c("id", "date")) %>%
  drop_na()
```
